...
function runTheme(options, req, res) {
  ...
  var iframeResizerJs = '//cdnjs.cloudflare.com/ajax/libs/iframe-resizer/3.5.15/iframeResizer.contentWindow.min.js';
  ...
  Promise.resolve()
    ...
    .then(function (theme) {
      ...
      return theme.render(options.resume, options.themeOptions);
    })
    .then(function (rendered) {
      rendered = rendered.replace('</head>','<script src="'+iframeResizerJs+'"></script></head>');

      // Replace it several times.  Too lazy to implement a replaceAll
      rendered = rendered.replace('http://fonts.googleapis.com','//fonts.googleapis.com');
      rendered = rendered.replace('http://fonts.googleapis.com','//fonts.googleapis.com');
      rendered = rendered.replace('http://fonts.googleapis.com','//fonts.googleapis.com');
      rendered = rendered.replace('http://fonts.googleapis.com','//fonts.googleapis.com');
      //rendered = rendered.replace(new RegExp('http://','g'),'//');
      // rendered = rendered.replace(new RegExp('https://','g'),'//');
      res.send(rendered);
    })
    .catch(function (error) {
      ...
      fs.readFileAsync(path.join(root, themeDirectory, 'package.json'), 'utf8').then(function (themePackageJson) {
        if (debug) {
          ...
          themePackageJson = JSON.parse(themePackageJson);
          let theme_homepage = null;
          if (themePackageJson.homepage) {
            theme_homepage = themePackageJson.homepage;
          }
          if (
            themePackageJson.repository
              && themePackageJson.repository.url
              && themePackageJson.repository.url.startsWith('http')
          ) {
            theme_homepage = themePackageJson.repository.url;
          }
...
var getTheme = function(req, res) {
  // Don't load the sample Richard Hendricks resume.
  //var resumeObject = _.cloneDeep(resume);
  var resumeObject = null;
  ...
  else if (req.body && req.body.resume_json) {
    console.log('Use posted resume json string');
    resumeObject = JSON.parse(req.body.resume_json);
  }
