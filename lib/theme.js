...
var performUnsafeRender = function(theme, data, cb) {
  try {
...
// Asyncronously process the request, returning the rendered HTML in the callback
  ...
  getThemeHtml(themeData, resumeObject, function(error, html) {
    ...
    augmentThemeData(themeData, function(error, themeData) {
      ...
      getThemeHtml(themeData, resumeObject, function(error, html) {
        ...
        downloadTheme(themeData, function(error) {
          if (error) {
            return cb(error);
          }
